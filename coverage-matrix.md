...
| Maps | なし | ☆☆☆ | 未着手 |
| Dependency Injection | なし | ★★☆ | 独立章はないが、[GreetingController.java](/C:/apps/learn-java-with-tests/src/main/java/dev/learnjavawithtests/springhttp/v1/GreetingController.java:1) で constructor injection を使用 |
| Mocking | なし | ☆☆☆ | 未着手 |
| Concurrency | なし | ☆☆☆ | 未着手 |
...
`learn-go-with-tests` の章ベースでは、現状は「基礎前半と HTTP の入口まで」が Java 化できている段階です。

優先度を付けるなら、次は `Maps`、`Dependency Injection`、`Mocking`、`IO`、`Concurrency` の順で埋めると教材としてつながりが良くなります。
